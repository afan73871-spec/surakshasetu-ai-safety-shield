        ...
        <>
          {activeTab === 'VIDEO' ? (
            <div className="space-y-6">
              ...
              {videoResult && (
                <div className={`w-full p-8 rounded-[56px] border-4 shadow-2xl animate-in zoom-in-95 space-y-10 relative overflow-hidden ${videoResult.isDeepfake ? 'bg-red-50 border-red-400' : 'bg-emerald-50 border-emerald-400'
                  }`}>
                  ...
                  <div className="flex items-center gap-6 relative z-10">
                    <div className={`p-6 rounded-[32px] shadow-xl text-white ${videoResult.isDeepfake ? 'bg-red-600 animate-pulse' : 'bg-emerald-600'
                      }`}>
                      {videoResult.isDeepfake ? <XCircle size={48} /> : <CheckCircle2 size={48} />}
                    </div>
                    <div>
                      <h3 className={`text-3xl font-black uppercase tracking-tighter leading-none ${videoResult.isDeepfake ? 'text-red-700' : 'text-emerald-700'
                        }`}>
                    ...
                    {[
                      ...
                      <div key={i} className="bg-white/80 backdrop-blur-md p-5 rounded-[28px] border border-black/5 flex items-center justify-between shadow-sm">
                        <div className="flex items-center gap-4">
                          <div className={`p-2.5 rounded-xl ${f.val.includes('DETECTED') || f.val === 'MISMATCHED' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'
                            }`}>
                            <f.icon size={18} />
                          </div>
                          <span className="text-[11px] font-black uppercase tracking-tight text-slate-700">{f.label}</span>
                        </div>
                        <span className={`text-[9px] font-black px-3 py-1 rounded-full uppercase ${f.val.includes('DETECTED') || f.val === 'MISMATCHED' ? 'bg-red-600 text-white' : 'bg-emerald-600 text-white'
                  ...
                  <button
                    onClick={() => {
                      const event = new CustomEvent('ask-suraksha-ai', {
                        detail: { question: `Explain the forensic findings for this video: "${videoResult.summary}". Is it safe to trust?` }
                      });
                  ...
                  <button
                    ...
                    className={`w-full py-6 rounded-[36px] font-black text-[12px] uppercase tracking-[0.4em] shadow-xl active:scale-95 transition-all relative z-10 ${videoResult.isDeepfake ? 'bg-red-600 text-white shadow-red-200' : 'bg-slate-900 text-white shadow-indigo-900/20'
                      }`}
                  >
              ...
              <div className="relative w-full aspect-[4/5] rounded-[56px] overflow-hidden border-8 border-white shadow-2xl bg-slate-950">
                ...
                {liveResult && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center px-10 text-center animate-in zoom-in duration-500 z-40 bg-slate-900/60 backdrop-blur-md">
                    <div className={`p-8 rounded-[48px] shadow-2xl mb-8 transform transition-transform scale-110 border-4 border-white/20 ${liveResult.type === 'REAL' ? 'bg-green-500' : liveResult.type === 'FAKE' ? 'bg-red-500' : 'bg-amber-500'
                      }`}>
                    ...
                    <button
                      onClick={() => {
                        const event = new CustomEvent('ask-suraksha-ai', {
                          detail: { question: `Explain the biometric liveness scan result: "${liveResult.reason}". What does this mean for my security?` }
                        });
                        window.dispatchEvent(event);
                      }}
                      className="w-full py-4 bg-white/10 border border-white/20 rounded-[28px] text-[10px] font-black uppercase tracking-widest text-white hover:bg-white/20 transition-all flex items-center justify-center gap-2 shadow-sm mt-4"
                    >
                      <BrainCircuit size={16} /> Deep AI Explainer
                    </button>
