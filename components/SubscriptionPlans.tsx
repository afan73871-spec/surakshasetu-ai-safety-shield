...
import { 
  ...
  // Added Activity icon to fix missing name error on line 267
  Activity
} from 'lucide-react';
...
export const SubscriptionPlans: React.FC<SubscriptionPlansProps> = ({ onSelectPlan }) => {
  ...
  // Poll for activation pulse from the cloud hub
  useEffect(() => {
    let interval: any;
    if (step === 'PENDING') {
    ...
    return (
      <div className="p-6 space-y-10 animate-in fade-in duration-700 pb-32 font-sans text-slate-900">
        <header className="text-center space-y-3">
           ...
           <div>
             <h2 className="text-4xl font-black uppercase tracking-tighter">Premium Access</h2>
             <p className="text-indigo-600 font-black text-[10px] uppercase tracking-[0.4em] mt-2">Maximum Neural Security</p>
           </div>
        </header>

        <div className="space-y-6">
           <div className="bg-white rounded-[44px] p-8 border-4 border-slate-900 shadow-2xl relative overflow-hidden group">
              ...
              <div className="relative z-10 space-y-8">
                 ...
                 <div className="space-y-4">
                    {[
                      'Unlimited Deepfake Forensic Audits',
                      'Real-time Voice Intent Interception',
                      'AI Malware Sandbox for APKs',
                      'Priority Neural Support Pulse',
                      'Global 5.2M Threat Registry Access'
                    ].map((feature, i) => (
                      <div key={i} className="flex items-center gap-3">
                         <div className="p-1 bg-green-100 text-green-600 rounded-full"><Check size={12} strokeWidth={4} /></div>
                 ...
                 >
                   Upgrade Hub <ChevronRight size={20} />
                 </button>
              </div>
           </div>

           <div className="bg-white rounded-[40px] p-8 border border-slate-100 shadow-xl space-y-6 opacity-60">
              <div className="flex justify-between items-center">
                 <h4 className="text-lg font-black uppercase text-slate-400">Standard</h4>
                 <span className="text-[10px] font-black uppercase bg-slate-100 px-3 py-1 rounded-full text-slate-400">Active</span>
              </div>
              <div className="space-y-3">
                 <div className="flex items-center gap-3 text-slate-400">
                    <Check size={14} />
                    <p className="text-xs font-bold">5 Scans Daily Limit</p>
                 </div>
                 <div className="flex items-center gap-3 text-slate-300">
                    <X size={14} />
                    <p className="text-xs font-bold line-through">Real-time Call Monitoring</p>
                 </div>
              </div>
           </div>
        </div>
    ...
    return (
      <div className="p-6 space-y-8 animate-in slide-in-from-bottom duration-500 pb-32">
        ...
        <div className="bg-white rounded-[48px] p-10 shadow-2xl border border-slate-100 flex flex-col items-center space-y-8 relative overflow-hidden">
           ...
           />
           <div className="text-center space-y-2">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Upgrade Fee</p>
           ...
           <div className="w-full space-y-4">
              <div className="flex items-center gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                 ...
                 <div className="flex-1 min-w-0">
                    <p className="text-[8px] font-black text-slate-400 uppercase">Hub UPI Handle</p>
           ...
           <div className="w-full space-y-4 pt-4">
              ...
              >
                ...
                {isBroadcasting ? 'BROADCASTING...' : 'SIGNAL PAID'}
              </button>
        ...
        <div className="bg-indigo-50 p-8 rounded-[40px] border border-indigo-100 flex items-start gap-4">
           ...
           <p className="text-[10px] text-indigo-900/60 font-bold uppercase tracking-wider leading-relaxed">
             <b>Handshake Protocol:</b> After transferring via UPI, broadcast the signal. The command center will verify the transaction within minutes and flip your neural switch to Pro.
           </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="p-8 flex flex-col items-center justify-center min-h-[60vh] text-center space-y-10 animate-in zoom-in-95">
         ...
         <div className="space-y-4">
            ...
            <p className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.4em] animate-pulse">Awaiting Central Confirmation</p>
            <div className="max-w-[240px] mx-auto bg-white/50 backdrop-blur-md p-6 rounded-[32px] border border-slate-100 shadow-sm mt-8">
               <p className="text-xs font-bold text-slate-500 leading-relaxed italic">
                 "Our Command Hub is auditing your payment signal. Your Neural Pro Shield will activate automatically once the audit is complete."
               </p>
            </div>
         </div>
         <button onClick={() => setStep('PLANS')} className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-300">Hub Dashboard</button>
      </div>
    );
  }
