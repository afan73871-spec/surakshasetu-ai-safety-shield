...
import {
  ...
  Terminal, RefreshCw, LogOut, ChevronLeft,
  Fingerprint, Zap, Lock, ShieldCheck,
  Radio, RefreshCcw, UserCheck, Trash2, Database,
  History, Filter, Activity, Globe, Receipt
} from 'lucide-react';
// Added SupportRequest to the imports from ../types
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onExit }) => {
  ...
  useEffect(() => {
    ...
    if (isAuthorized) {
      loadData();
      interval = setInterval(() => loadData(true), 10000); // 10s sync pulse
    }
  ...
  const handleAdminAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage(err.message || "COMMANDER IDENTITY REJECTED");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="relative z-10 w-full max-w-sm space-y-10 animate-in zoom-in duration-500">
          ...
          <form onSubmit={handleAdminAuth} className="space-y-4">
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 px-6 outline-none focus:border-indigo-600 font-bold text-white"
              required
            />
            <input
              type="password"
            ...
            <button disabled={isLoading} type="submit" className="w-full py-6 bg-indigo-600 rounded-2xl font-black text-xs uppercase tracking-[0.4em] active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
              {isLoading ? 'Authorizing...' : 'Authorize Hub'}
            </button>
    ...
    <div className="min-h-screen bg-slate-950 text-white font-sans flex flex-col">
      ...
      <div className="bg-slate-900/40 backdrop-blur-3xl border-b border-white/5 px-8 py-6 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-xl font-black uppercase tracking-tighter">Signal Dashboard</h2>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setActiveTab('LEDGER')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'LEDGER' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white/5 text-slate-500'}`}>Registry Ledger</button>
        </div>
      </div>
          ...
          <div className="bg-slate-900 border border-white/10 rounded-[40px] p-8 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-indigo-600/20 rounded-2xl flex items-center justify-center text-indigo-400">
                {activeTab === 'QUEUE' ? <Activity size={24} /> : <Database size={24} />}
              </div>
              <div>
                <h3 className="text-xl font-black uppercase tracking-tight">{activeTab === 'QUEUE' ? 'Activation Queue' : 'Central Registry'}</h3>
                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{filteredRequests.length} Signals Identifed</p>
              </div>
            </div>
            <div className="relative w-full md:w-64">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-600" size={16} />
              <input type="text" placeholder="Filter by email..." value={filter} onChange={(e) => setFilter(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-2xl py-4 pl-12 pr-6 outline-none focus:border-indigo-600 font-bold text-xs" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredRequests.length === 0 ? (
              <div className="text-center py-20 opacity-20 space-y-4">
                ...
                <p className="font-black uppercase tracking-[0.5em] text-xs">No Signal Data Synchronized</p>
                <button onClick={() => loadData()} className="text-[10px] font-black text-indigo-400 underline uppercase">Force Pulse Refresh</button>
              </div>
              ...
              <div key={req.id} className="bg-slate-900/60 border border-white/10 rounded-[32px] p-6 relative overflow-hidden group hover:bg-slate-900 transition-all">
                {transmittingId === req.id && (
                  <div className="absolute inset-0 z-20 bg-indigo-600/90 backdrop-blur-md flex items-center justify-center gap-4 animate-in fade-in duration-200">
                    ...
                    <h4 className="font-black uppercase tracking-tighter">Syncing Identity Hub...</h4>
                  </div>
                )}
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                  <div className="flex items-center gap-5 text-center md:text-left flex-col md:flex-row">
                    ...
                    <div>
                      <div className="flex items-center gap-3 justify-center md:justify-start">
                        ...
                        <span className={`text-[8px] font-black px-2 py-0.5 rounded-full border ${req.status === 'PENDING' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' :
                      ...
                      <p className="text-[8px] text-slate-600 font-bold uppercase mt-0.5">Handshake Node: {req.utr}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 w-full md:w-auto">
                    {req.status === 'APPROVED' && (
                      ...
                      >
                        <Receipt size={20} />
                        <span className="text-[9px] font-black uppercase tracking-widest hidden md:inline">Invoice</span>
                      </button>
                    )}
                    {req.status === 'PENDING' && (
                      <button onClick={() => handleApprove(req.id)} className="flex-1 md:flex-none py-4 px-8 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-600/20 active:scale-95 transition-all hover:bg-indigo-500">AUTHORIZE</button>
                    )}
      ...
      <footer className="p-6 text-center bg-slate-900 border-t border-white/5 fixed bottom-0 left-0 w-full">
        <p className="text-[8px] font-black uppercase tracking-[0.5em] text-slate-600">Edwin Commander Node • Secure Registry Session</p>
      </footer>
    </div>
  );
};