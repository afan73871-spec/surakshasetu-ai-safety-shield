...
import { 
  ...
  // Added comment above fix: Added missing icons BrainCircuit and Waves
  BrainCircuit, Waves
} from 'lucide-react';
...
export const DeepfakeScanner: React.FC = () => {
  ...
  const runVideoAnalysis = async () => {
    ...
    setIsScanning(true);
    setProgress(0);
    setStatusMessage("Decompressing Video Buffers...");
    ...
    <div className="flex flex-col p-5 space-y-6 pb-40 animate-in fade-in duration-700 font-sans min-h-screen bg-slate-50">
      ...
      <header className="w-full flex justify-between items-center px-1">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">Neural Intelligence Core v5.0</p>
          </div>
        </div>
      ...
      {showHistory ? (
        <div className="w-full space-y-4 animate-in slide-in-from-right-4 duration-300">
          <div className="flex justify-between px-2"><h3 className="text-[10px] font-black uppercase text-slate-400">Registry Records</h3><button onClick={() => setHistory([])} className="text-[10px] font-black text-red-500">Wipe Hub</button></div>
          {history.length === 0 ? (
              ...
              {!selectedVideo && !videoResult && (
                ...
                >
                   ...
                   <div className="space-y-2">
                     ...
                     <p className="text-xs text-slate-400 font-bold uppercase tracking-widest max-w-[200px] mx-auto">Analyze WhatsApp/Instagram clips for deepfake logic</p>
                   </div>
                ...
                <div className={`w-full p-8 rounded-[56px] border-4 shadow-2xl animate-in zoom-in-95 space-y-10 relative overflow-hidden ${
                  ...
                  <div className="grid grid-cols-1 gap-3 relative z-10">
                     {[
                       ...
                       { label: 'Generation Artifacts', val: videoResult.findings.generationArtifacts, icon: Zap }
                     ].map((f, i) => (
