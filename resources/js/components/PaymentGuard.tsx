...
import { 
  ...
  Search, ShieldAlert, Info, Smartphone, User, CheckCircle2, Zap
} from 'lucide-react';
...
export const PaymentGuard: React.FC<PaymentGuardProps> = ({ isPro }) => {
  ...
  const [credits, setCredits] = useState(usageService.getRemainingCredits(isPro).upi);

  useEffect(() => {
    const updateCredits = () => setCredits(usageService.getRemainingCredits(isPro).upi);
  ...
  const handleAudit = async () => {
    ...
    if (!usageService.canUseFeature('UPI_SAFE', isPro)) {
      alert("Daily UPI Safety Limit (5) Reached. Upgrade to Pro for unlimited merchant verification.");
    ...
    try {
      ...
      setAnalysis(result);
      if (!isPro) usageService.incrementUsage('upi');
    } catch (err) {
  ...
  return (
    <div className="p-5 space-y-6 animate-in fade-in duration-700 pb-32 font-sans text-black">
      ...
      <div className="space-y-4">
        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] px-2">Quick Test Hub</h4>
      ...
      <div className="bg-indigo-50 p-6 rounded-[32px] border border-indigo-100 flex items-start gap-4">
        ...
        <p className="text-[10px] text-indigo-900/60 font-bold uppercase tracking-wider leading-relaxed">
          AI Safe Pay Audit cross-references global merchant reputations and recent scam patterns to prevent fraudulent transfers.
        </p>
      </div>
