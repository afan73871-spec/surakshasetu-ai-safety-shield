...
import { 
  ChevronLeft, Cpu, Zap, Shield, BrainCircuit, 
  Upload, FileArchive, RefreshCw, AlertTriangle, 
  CheckCircle2, XCircle, Info, Loader2, Trash2, FileText
} from 'lucide-react';
import { AdBanner } from './AdBanner';
import { analyzeFileSafety, FileSafetyResult } from '../services/geminiService';
import { usageService } from '../services/usageService';

export const APKScanner: React.FC = () => {
  const [file, setFile] = useState<File | null>(null);
  const [isScanning, setIsScanning] = useState(false);
  const [scanResult, setScanResult] = useState<FileSafetyResult | null>(null);
  ...
  useEffect(() => {
    ...
    setIsPro(pro);
    setCredits(usageService.getRemainingCredits(pro).apk);

    const updateCredits = () => setCredits(usageService.getRemainingCredits(pro).apk);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected) {
      ...
      if (name.endsWith('.apk') || name.endsWith('.pdf')) {
        setFile(selected);
        setScanResult(null);
      } else {
        alert("Invalid format. Please upload a .apk or .pdf file.");
      }
    }
  };
  ...
  const startAnalysis = async () => {
    ...
    if (!usageService.canUseFeature('APK_SCANS', isPro)) {
      alert("Daily File Audit Limit (5) Reached. Upgrade to Pro for unlimited malware scanning.");
    ...
    const steps = isPdf ? [
      ...
      "Searching for Embedded JavaScript...",
      "Analyzing PDF Stream Metadata...",
      "Syncing with Document Threat Hub...",
    ...
    try {
      const result = await analyzeFileSafety(file.name, file.size);
      setScanResult(result);
      if (!isPro) usageService.incrementUsage('apk');
    } catch (e) {
  ...
  return (
    <div className="p-5 space-y-6 animate-in slide-in-from-right duration-500 pb-32 font-sans flex flex-col items-center">
      <header className="w-full flex flex-col items-center relative text-center space-y-4">
        ...
        <div>
          <h2 className="text-3xl font-black uppercase tracking-tighter text-slate-900 leading-none">Binary & Doc Shield</h2>
      ...
      {!file && !scanResult && (
        ...
        >
           <input type="file" ref={fileInputRef} onChange={handleFileChange} accept=".apk,.pdf" className="hidden" />
           ...
           <div className="space-y-1">
             <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">Upload File</h3>
             <p className="text-xs text-slate-400 font-bold uppercase tracking-widest leading-relaxed">Audit APK or PDF documents</p>
           </div>
           <div className="flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-2xl border border-slate-100">
              <FileArchive size={14} className="text-slate-400" />
              <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Supports .apk and .pdf</span>
           </div>
        </div>
      )}
          ...
          <div className="flex items-center justify-between bg-slate-50 p-4 rounded-[28px] border border-slate-100">
             <div className="flex items-center gap-4">
               <div className="p-3 bg-indigo-100 text-indigo-600 rounded-xl">
                 {isPdf ? <FileText size={24} /> : <FileArchive size={24} />}
               </div>
        ...
        <div className={`w-full p-8 rounded-[48px] border-4 shadow-2xl animate-in zoom-in-95 space-y-8 relative overflow-hidden ${
          ...
          <div className="flex items-center gap-5 relative z-10">
             ...
             <div>
               ...
               <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mt-1.5 flex items-center gap-2">
                 ...
                 {scanResult.isSecure ? 'File Integrity Verified' : 'Threat Signature Detected'}
               </p>
             </div>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm p-6 rounded-[32px] border border-black/5 space-y-4 relative z-10">
             ...
             <div className="grid grid-cols-2 gap-4 pt-2">
                <div className="space-y-1">
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">{isPdf ? 'Script Risk' : 'Permissions Risk'}</p>
                ...
                <div className="space-y-1">
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Scan Confidence</p>
                  <p className="text-[10px] font-black text-indigo-600 uppercase">98.4% Neural Link</p>
                </div>
             </div>

             {scanResult.detectedMalware.length > 0 && (
               <div className="pt-4 border-t border-slate-100">
                  <p className="text-[9px] font-black text-red-500 uppercase tracking-widest mb-2">Isolated Exploit Vectors</p>
      ...
      <div className="w-full max-w-sm bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm space-y-6">
         <div className="flex items-center gap-4">
            ...
            <div>
               <h4 className="text-sm font-black uppercase text-slate-900 leading-none">Unified Malware Shield</h4>
               <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">Deep Bytecode & Doc Logic Audit</p>
            </div>
         </div>
         <p className="text-xs text-slate-500 leading-relaxed font-medium">
            Shield executes suspicious APKs and PDFs in a <strong>Secure Neural Sandbox</strong> to analyze their behavior before they can compromise your identity or device.
         </p>
      </div>
      ...
      <div className="bg-amber-50 p-6 rounded-[36px] border border-amber-100 flex items-start gap-4">
        ...
        <p className="text-[10px] text-amber-900/60 font-bold uppercase tracking-wider leading-relaxed">
          <b>Heuristic Note:</b> Malicious PDFs often use obfuscated scripts to steal data. Our scanner de-obfuscates document streams to check for known exploit signatures.
        </p>
      </div>
