...
export const Auth: React.FC<AuthProps> = ({ onLogin, onGuestLogin }) => {
  ...
  useEffect(() => {
    const initializeGoogle = () => {
      try {
        ...
        console.warn("Auth Link Pulse Weak.");
      }
    };
  ...
  const handleGoogleResponse = async (response: any) => {
    ...
    try {
      ...
      if (result.success && result.user) {
        ...
        setErrorMessage("Identity Hub: Sync Failed.");
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage("Registry Offline. Check Connection.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (isSuccess) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-8 text-white font-sans">
        ...
        <h2 className="text-3xl font-black mb-2 uppercase tracking-tighter text-center">Neural Link Active</h2>
  ...
  return (
    <div className="min-h-screen bg-indigo-950 flex flex-col p-8 font-sans relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col h-full max-w-sm mx-auto w-full pt-12">
        ...
        <div className="bg-white/5 p-1.5 rounded-[28px] border border-white/10 backdrop-blur-md mb-8 flex">
           <button 
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
           ...
           {errorMessage && (
             <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-[20px] text-red-400 text-[9px] font-black uppercase tracking-widest text-center flex items-center justify-center gap-2 animate-in shake">
