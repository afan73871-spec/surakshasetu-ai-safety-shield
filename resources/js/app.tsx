...
// Fixed: Removed non-existent ActiveCall import
...
import { 
  ...
  BadgeCheck, Info, Waves, Flag, Rocket, Star
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case AppView.PRIVACY_GUARD: return <PrivacyGuard isPro={true} />;
      ...
      // Added isPro prop to SupportPage fix line 145 error
      ...
      default:
        return (
          <div className="p-5 space-y-6 animate-in fade-in duration-500 pb-24 text-black relative">
             ...
             <div className="bg-gradient-to-br from-[#128807] via-emerald-800 to-slate-900 rounded-[44px] p-10 text-white shadow-2xl flex items-center justify-between border border-emerald-400/20">
               <div className="flex items-center gap-5">
                 <div className="p-4 bg-white/10 rounded-[28px] backdrop-blur-md relative">
               ...
               <Star className="text-yellow-300 animate-pulse" size={32} fill="currentColor" />
             </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button onClick={() => setCurrentView(AppView.DEEPFAKE_SCAN)} className="bg-white p-6 rounded-[36px] border border-gray-100 shadow-sm hover:border-indigo-200 transition-all active:scale-95 text-left group border-t-4 border-t-[#FF9933]">
                <div className="p-4 bg-red-50 text-red-600 rounded-2xl w-fit mb-5 group-hover:bg-red-600 group-hover:text-white transition-all"><Video size={32} /></div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button onClick={() => setCurrentView(AppView.URL_SHIELD)} className="bg-white p-6 rounded-[36px] border border-gray-100 shadow-sm hover:border-indigo-200 transition-all active:scale-95 text-left group">
                <div className="p-4 bg-indigo-50 text-indigo-600 rounded-2xl w-fit mb-5 group-hover:bg-indigo-600 group-hover:text-white transition-all"><Zap size={32} /></div>
                <h3 className="font-black text-gray-800 text-lg leading-tight mb-1">Web Shield</h3>
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Safe Surfing</p>
              </button>
              <button onClick={() => setCurrentView(AppView.APK_SCANNER)} className="bg-white p-6 rounded-[36px] border border-gray-100 shadow-sm hover:border-indigo-200 transition-all active:scale-95 text-left group">
                <div className="p-4 bg-slate-900 text-white rounded-2xl w-fit mb-5 group-hover:bg-indigo-600 transition-all"><Zap size={32} /></div>
                <h3 className="font-black text-gray-800 text-lg leading-tight mb-1">APK Shield</h3>
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Malware Check</p>
              </button>
            </div>
            ...
            <button onClick={() => setCurrentView(AppView.SCAM_DATABASE)} className="w-full bg-slate-950 rounded-[40px] p-8 text-white flex items-center justify-between shadow-2xl active:scale-95 transition-all group overflow-hidden relative">
              ...
              <div className="flex items-center gap-6 relative z-10">
                ...
                <div className="text-left"><h3 className="font-black text-2xl tracking-tighter uppercase">Registry Hub</h3><p className="text-[10px] text-[#FF9933] font-bold uppercase tracking-widest">5.2M Scammer Profiles</p></div>
              </div>
  ...
  return (
    <div className="min-h-screen max-w-md mx-auto bg-slate-50 flex flex-col shadow-2xl overflow-hidden relative font-sans">
      <div className="bg-white px-6 py-5 flex items-center justify-between border-b border-gray-100 sticky top-0 z-40 backdrop-blur-md bg-white/80">
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[7px] font-black uppercase tracking-[0.4em] text-[#FF9933] mt-1">National Cyber AI</span>
          </div>
        </div>
      ...
      <nav className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[92%] max-w-sm bg-white/80 backdrop-blur-2xl border border-white/50 shadow-[0_25px_60px_rgba(0,0,0,0.18)] rounded-[36px] flex justify-around p-3 z-40">
        {NAV_ITEMS.map(item => (
          <button key={item.id} onClick={() => setCurrentView(item.id as AppView)} className={`flex flex-col items-center p-3.5 rounded-[22px] transition-all duration-300 ${currentView === item.id ? 'text-indigo-600 scale-110 bg-indigo-50/50' : 'text-gray-400 hover:text-gray-600'}`}>
            {item.icon}
            <span className="text-[9px] mt-1.5 font-black uppercase tracking-[0.2em]">{item.label}</span>
          </button>
        ))}
      </nav>
